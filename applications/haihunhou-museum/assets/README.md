...
### Haihunhou Museum Chinese subset

- `fonts/haihunhou_zh_16.c` is a 16 px, 2 bpp LVGL subset generated from Noto
...
- Regenerate from the visible Chinese strings in `main/haihunhou_museum.c` with
...
### Haihunhou Museum README artwork

- `images/haihunhou-museum-cover.png` is the 1086 × 1448 community cover created
...
- `images/haihunhou-museum-runtime.png` is a 240 × 320 framebuffer capture of the
  goose-and-fish lamp stop. It contains only the app interface and no device or account
  identifiers.
...
### Haihunhou Museum offline guide narration

- `music/haihunhou_tts/` stores 22 source WAV files in page order, named from
  `01_site_story.wav` through `22_location_detail.wav`.
- All clips use project-authored narration synthesized by
  `tools/generate_haihunhou_tts.py` with the macOS Tingting system voice. No
...
- WAV sources are 16 kHz, 16-bit mono PCM. The same script compresses and packs
  them as 4-bit IMA-ADPCM in `main/haihunhou_tts_audio.c`.
