...
parser.add_argument('--input', type=Path, default=Path('/tmp/deadline-station-previews'))
args = parser.parse_args()
output = ROOT / 'projects/deadline-station/assets'
output.mkdir(parents=True, exist_ok=True)
for p in args.input.glob('*.ppm'):
    with Image.open(p) as im:
        im.save(output / (p.stem + '.png'))
names = ['vision', 'home', 'detail', 'checklist', 'year', 'focus']
...
ImageDraw.Draw(canvas).text((16, 5), '截稿小站 · 本机模拟预览（非设备截图）', font=font, fill='#17202a')
