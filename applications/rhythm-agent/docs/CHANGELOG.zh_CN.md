...
- 训练按对顺序即可过关且不设输入超时；节奏按相邻两拍独立评分，避免累积偏差，并明确重试原因。增加慢速训练和节奏漂移回归测试。
- 屏幕按键与操作提示按实机从上到下的“上／下／确定”排列。
- 新增离线中文节奏游戏，含训练与挑战、同题重赛、普通话提示、输入时间戳评分与挑战纪录保存。
- 新增电脑界面排版检查与音频流测试，保留串口截图能力供后续真机验收。
