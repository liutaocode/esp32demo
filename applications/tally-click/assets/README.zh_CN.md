...
三个 `tally_zh` 字体源码为思源黑体简体中文版 2bpp 子集，由 `tools/generate_tally_fonts.py` 生成，采用 SIL OFL 1.1，授权文本位于 `fonts/tally-licenses/`。1.2 版以 LVGL 几何直接绘制原创可缩放数码字和特效，音效由 `tools/generate_tally_audio.py` 离线原创生成，没有嵌入位图或录音。早期 `tally_digits_64.c` 的 Montserrat 子集及其授权保留为未使用的 1.0 版源码素材。
