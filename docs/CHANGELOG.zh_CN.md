...
- 获取网络地址后开始校时，配置三个 NTP 校时源自动切换。
- 等待 10 秒后明确提示“校时超时，正在重试”，立即切换下一校时源，每 10 秒重试，并保留 TLS 证书验证。

## Qwen 语音豆稳定性更新 — 2026-09-10

- 首页增加音量及静音提示，避免表情裁剪占用过多内存。
- 改善重连、音频发送节奏与 TLS/Wi-Fi 内存使用，保持上游心跳及时处理。
- 支持公网 IP 加密连接，不预置个人服务器配置；仍可能出现网络发送超时。

## 嘴硬小豆源码包 — 2026-09-08

- 新增独立 Qwen 语音豆示例，支持手机配网、Gateway 语音传输和麦克风暂停/恢复。
- 新增独立的 `examples/mouthy-bean` 1.0.1 源码包、测试、语音资源和双语说明，复用现有板级支持，不改变其他应用选择器。社区项目 220 的说明注明灵感来自「小狮日记」，不提交设备数据或生成固件。

## 2026-09-08

- 仓库 README 默认显示中文，保留英文切换入口。
- 整理 40 个社区已发布应用的源码索引、构建入口及独立工程；排除本地审核记录和设备回执。
- 将已发布应用的新增主机测试纳入验证，并更新归档技能中的隐私检查、源码范围与目标分支规则。

## Unreleased

- 上传独立口袋书架 TTS 版源码，包含三本测试书、连续离线朗读及菜单音量调节。

- 收录 20 个已送审应用，并保留我的世界语音图鉴；支持通过 `FAP_APP` 独立构建各应用，保留状态机、音频与资源测试。
- 提交内容排除未送审草稿、发布回执、设备记录和本机路径。

- 将 BSP 演示菜单替换为开机直达的“我的世界图鉴”，内含 20 个像素插图条目；
  上下键循环切换并播报新条目，确定键重播当前介绍，界面保留电量显示，音频由工作
  任务从紧凑的内嵌 IMA-ADPCM 资源分块解码播放。
- 将界面统一为《我的世界》风格的方块物品栏布局，电池百分比固定单行显示，并把
  上一个、确定、下一个整理为三个紧凑等宽按键。
- 增加社区发布所需的只读串口截屏能力，可从正在运行的图鉴界面生成真实画面凭证。
