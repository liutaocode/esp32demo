...
- Each key has a distinct generated tone. The vertical on-screen pads follow
  physical UP / OK / DOWN order. Firmware input uses immediate PRESS events;
...
Every key must be correct, and the mean rhythm score must reach 60 in training or
75 in challenge. Correct notes score 100 within 110/80 ms of the target,
75 within 220/160 ms, otherwise 25; wrong or missed notes score zero.
Times are relative to the first press. The input timestamp is captured before
queueing, so render delays do not change the score. No first-press timeout;
after starting, a note more than 1100 ms late ends the attempt. A passed lock
adds its accuracy to the total, up to 800. Failed attempts add no points.
A four-digit task code seeds the same patterns for same-mode replays on this version.

Build and source: [application details](main/apps/rhythm_agent/README.md).
