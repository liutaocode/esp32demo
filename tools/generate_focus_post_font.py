#!/usr/bin/env python3
"""Regenerate Focus Post's Chinese font subsets from its UI source."""
...
source = (ROOT / "main/apps/focus_post/focus_post.c").read_text()
symbols = "".join(sorted({ch for ch in source if ord(ch) > 127}))
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--check", action="store_true")
args = parser.parse_args()
for size in (16, 24):
    name = f"focus_post_zh_{size}"
    ...
    if args.check:
        ...
        print(f"Focus Post {size}px font: {len(symbols)} UI characters covered")
        continue
    subprocess.run([
