...
- `fonts/cat_zh_{12,14,16,20}.c`：由 `tools/cat/font.py` 从锁定 LVGL 中的思源黑体中文字体生成，使用 SIL OFL 1.1 许可，见 `fonts/OFL-SourceHanSans.txt`。
- 原创小猫、房间、玩具与爱心由 `main/cat/cat_ui.c` 绘制，没有下载的位图素材。
- `music/cat/`：Joseph SARDIN / BigSoundBank 的 CC0 猫咪呼噜和叫声录音，来源、处理及哈希见 `music/cat/audio-manifest.json`，由 `tools/cat/audio.py` 生成十段 IMA ADPCM：三种猫叫、三段呼噜、三种原创玩具声和一段八秒循环、每分钟一百二十拍的原创舞曲。压缩音库为 204,160 字节，WAV 文件用于试听。不含 TTS。
