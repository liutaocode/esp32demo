#!/usr/bin/env python3
"""Regenerate Mouthy Bean Online's Chinese font subsets from its UI source."""
...
source = "\n".join(p.read_text() for p in (ROOT / "main").glob("online_*.c"))
symbols = "".join(sorted({ch for ch in source if ord(ch) > 127}))
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--check", action="store_true")
args = parser.parse_args()
for size in (12, 16):
    name = f"mouthy_bean_zh_{size}"
    ...
    if args.check:
        ...
        print(f"Mouthy Bean Online {size}px font: {len(symbols)} UI characters covered")
        continue
    subprocess.run([
        "npx", "--yes", "lv_font_conv@1.5.3",
        "--size", str(size), "--bpp", "2", "--format", "lvgl",
        "--font", str(Path(__import__("os").environ.get("ONLINE_FONT_SOURCE", str(ROOT / "managed_components/lvgl__lvgl/scripts/built_in_font/SourceHanSansSC-Normal.otf")))),
