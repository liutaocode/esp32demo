...
`books/tang.txt` 和 `books/song.txt` 为诗词测试书，采用李白、孟浩然、王之涣、李清照、晏殊的公版作品，参考[维基文库](https://zh.wikisource.org/)转为简体中文。随机排列后的顺序固定在 UTF-8 文件中，避免影响阅读进度。CMake 内嵌数据，启动时仅补充一次缺少的示例书。
