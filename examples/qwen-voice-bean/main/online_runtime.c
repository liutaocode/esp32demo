...
/* Fixed queue budget ~30 KB, one bounded JSON frame <=32 KB. */
...
static void receive_message(void) {
    cJSON *j=cJSON_Parse(frame);if(!j){fail("后端消息格式错误");return;}
...
static bool send_json(cJSON *j) {
    ...
    if(!raw)return false;
    int length=strlen(raw);int sent=esp_websocket_client_send_text(gateway_socket,raw,length,pdMS_TO_TICKS(1000));free(raw);
    return sent==length;
}
...
static void send_hello(void) {
    ...
    cJSON_AddItemToArray(caps,cJSON_CreateString("input.audio"));cJSON_AddItemToArray(caps,cJSON_CreateString("playback.receipts"));
...
static void audio_task(void *arg) {
    ...
    for(;;) {
        if(esp_timer_get_time()-last_metrics>10000000) {
            ...
            ESP_LOGI("bean_audio","stack_free_min=%u heap_free=%lu ready=%d capture_drops=%u",
                (unsigned)uxTaskGetStackHighWaterMark(NULL),(unsigned long)esp_get_free_heap_size(),atomic_load(&ready),atomic_load(&capture_drops));
        }
        ...
        if(!audio_available || !atomic_load(&ready)){vTaskDelay(pdMS_TO_TICKS(20));continue;}
...
static void network_task(void *arg) {
    ...
    esp_websocket_client_config_t ws={.uri=config.url,.headers=config.token[0]?headers:NULL,.buffer_size=2048,.task_stack=6144,
        .network_timeout_ms=8000,.reconnect_timeout_ms=5000,.enable_close_reconnect=true,.crt_bundle_attach=esp_crt_bundle_attach};
    ...
    for(;;) {
        ...
        if(atomic_exchange(&fault,false)) {
            ...
            esp_websocket_client_start(gateway_socket);
        }
        ...
        if(atomic_load(&connected) && previous_mic!=atomic_load(&mic)) {
            ...
            if(previous_mic && !send_json(event("wake"))) {fail("恢复对话失败");continue;}
            if(!send_json(event(previous_mic?"input.unmute":"input.mute"))) {fail("麦克风状态同步失败");continue;}
        ...
        while(xQueueReceive(receipts,&r,0)==pdTRUE)if(atomic_load(&connected)) {
            cJSON *j=event(r.type);cJSON_AddStringToObject(j,"responseId",r.id);if(!send_json(j))fail("回复确认发送失败");
        }
        if(xQueueReceive(capture,&c,pdMS_TO_TICKS(10))==pdTRUE && atomic_load(&ready) && atomic_load(&mic) && c.epoch==atomic_load(&epoch)) {
