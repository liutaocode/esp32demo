#!/usr/bin/env python3
"""Regenerate Pocket Breach's Chinese font subsets from its UI source."""
...
source = (ROOT / "main/apps/pocket_breach/pocket_breach.c").read_text()
symbols = "".join(sorted({ch for ch in source if ord(ch) > 127}))
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--check", action="store_true")
args = parser.parse_args()
for size in (16,):
    name = f"pocket_breach_zh_{size}"
    ...
    if args.check:
        ...
        print(f"Pocket Breach {size}px font: {len(symbols)} UI characters covered")
        continue
    subprocess.run([
