#!/usr/bin/env python3
"""Regenerate Code Theater's Chinese font subsets from its UI source."""
...
source = "".join(p.read_text() for p in (ROOT / "main/apps/code_theater").glob("*.c"))
symbols = "".join(sorted({ch for ch in source if ord(ch) > 127}))
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--check", action="store_true")
args = parser.parse_args()
for size in (14, 16):
    name = f"code_theater_zh_{size}"
    ...
    if args.check:
        ...
        print(f"Code Theater {size}px font: {len(symbols)} UI characters covered")
        continue
