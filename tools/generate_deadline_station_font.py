#!/usr/bin/env python3
"""Regenerate Deadline Station's Chinese font subsets from its UI source."""
...
source = "".join(p.read_text() for p in (ROOT / "main/apps/deadline_station").glob("*.c"))
symbols = "".join(sorted({ch for ch in source if ord(ch) > 127}))
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--check", action="store_true")
args = parser.parse_args()
for size in (12, 16, 24):
    name = f"deadline_station_zh_{size}"
    ...
    if args.check:
        ...
        print(f"Deadline Station {size}px font: {len(symbols)} UI characters covered")
        continue
    subprocess.run(shlex.split(os.environ.get("LV_FONT_CONV", "npx --yes lv_font_conv@1.5.3")) + [
