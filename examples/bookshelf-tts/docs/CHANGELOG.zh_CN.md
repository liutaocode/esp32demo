...
- 保留朗读体验，新增唐诗选读、宋词选读两本测试书，保留已有文件与阅读进度。

- 增加阅读菜单内的音量调节，以及朗读时短暂显示的音量进度条。

- 新增独立“口袋书架（TTS版）”：连续中文朗读、按句暂停续读、语速控制、自动跟随翻页，
  并关闭翻页音效。朗读模式下上下键调音量，顶栏及状态栏自动收起。保留原书籍存储布局及阅读功能。

- 新增口袋朗读员：十二组离线中文 TTS 示例、六档语速、可取消播放与 USB 控制。
  内嵌 930 KB Speex 语音库，保留原发音词典；正常语速按帧流式播放，变速仅缓存
  单个音节；语音库保留 16 kHz 带宽，默认节奏在离线制作时调整。取消独立语音分区。
