...
static eb_speech_t s_speech = {.speed = 2};
static const char *s_speech_note;
...
static lv_obj_t *s_volume_box, *s_volume_bar, *s_volume_label;
...
static const char *MENU_TEXT[EB_MENU_COUNT] = { "开始朗读", "停止朗读", "朗读语速", "音量", "添加书签", "书签列表", "阅读统计", "字号", "主题", "返回书架" };
...
static void publish(void)
{
    ...
    view->volume = tts_runtime_volume();
    view->volume_until = s_volume_until;
    view->volume_edit = s_model.volume_edit;
    ...
    case EB_SCREEN_SHELF: {
        snprintf(view->head, sizeof(view->head), "口袋书架（TTS版）");
    ...
    case EB_SCREEN_READER: {
        ...
        build_page(view);
        if (s_speech.active || s_speech.paused || s_speech_note)
            snprintf(view->foot, sizeof(view->foot), "%d%% · %s · 确定菜单", percent,
                     s_speech_note ? s_speech_note : s_speech.active ? "朗读中" : "已暂停");
        if (s_volume_until > esp_timer_get_time())
            snprintf(view->foot, sizeof(view->foot), "音量 %u%% · 上加下减", tts_runtime_volume());
        break;
    }
    case EB_SCREEN_MENU:
        snprintf(view->head, sizeof(view->head), "阅读菜单");
        fill_list_window(view, EB_MENU_COUNT, s_model.menu_sel);
        for (int i = 0; i < view->rows; i++) {
            int item = s_list_top + i;
            const char *name = MENU_TEXT[item];
            if (item == EB_MENU_SPEECH) name = s_speech.active ? "暂停朗读" : s_speech.paused ? "继续朗读" : "开始朗读";
            snprintf(view->row_text[i], sizeof(view->row_text[i]), "%s", name);
            view->row_sub[i][0] = 0;
            if (item == EB_MENU_FONT) snprintf(view->row_sub[i], sizeof(view->row_sub[i]), "%s", FONT_TEXT[s_model.font]);
            else if (item == EB_MENU_THEME) snprintf(view->row_sub[i], sizeof(view->row_sub[i]), "%s", THEME_TEXT[s_model.theme]);
            else if (item == EB_MENU_MARKS) snprintf(view->row_sub[i], sizeof(view->row_sub[i]), "%d 条", s_model.mark_count);
            else if (item == EB_MENU_VOLUME) snprintf(view->row_sub[i], sizeof(view->row_sub[i]), "%u%%", tts_runtime_volume());
            else if (item == EB_MENU_SPEECH_SPEED) snprintf(view->row_sub[i], sizeof(view->row_sub[i]), "%u/5", s_speech.speed);
            else if (item == EB_MENU_STATS) eb_format_duration(s_model.read_seconds_book, view->row_sub[i], sizeof(view->row_sub[i]));
        }
        view->foot[0] = '\0';
        snprintf(view->hint, sizeof(view->hint), "%s", s_model.volume_edit ? "上下调音量 · 确定完成" : "上下选择 · 确定执行 · 长按确定回正文");
        if (s_model.volume_edit) snprintf(view->foot, sizeof(view->foot), "上下调音量 · 确定完成");
        break;
...
static void speech_stop(void) {
    eb_speech_stop(&s_speech); tts_runtime_stop(); s_speech_note = NULL;
}
static void speech_poll(void) {
    if (!s_speech.active) return;
    tts_snapshot_t status = tts_runtime_snapshot();
    if (status.status == TTS_LOADING || status.status == TTS_PREPARING || status.status == TTS_SPEAKING) return;
    if (status.status == TTS_ERROR) {
        eb_speech_stop(&s_speech); s_speech_note = "这段暂时无法朗读"; publish(); return;
    }
    if (s_speech.waiting) eb_speech_completed(&s_speech);
    uint32_t total = eb_fs_size();
    if (s_speech.cursor >= total) {
        eb_speech_stop(&s_speech); s_speech_note = "全书已读完"; publish(); save_progress(); return;
    }
    /* Follow the spoken position without changing the reader's pagination. */
    if (s_model.screen == EB_SCREEN_READER) {
        while (s_page_bytes && s_speech.cursor >= s_model.page_offset + s_page_bytes && !s_model.at_eof) {
            eb_state_advance(&s_model, s_model.page_offset + (uint32_t)s_page_bytes);
            mark_progress_dirty(); publish();
        }
    }
    char raw[1024], text[TTS_TEXT_BYTES];
    size_t n = eb_fs_read(s_speech.cursor, raw, sizeof(raw));
    size_t consumed = eb_speech_chunk(raw, n, text, sizeof(text));
    if (!consumed) { eb_speech_stop(&s_speech); s_speech_note = "正文读取失败"; publish(); return; }
    s_speech.next = s_speech.cursor + (uint32_t)consumed;
    if (!tts_text_valid(text)) { s_speech.cursor = s_speech.next; return; }
    esp_err_t err = tts_runtime_say(text, s_speech.speed);
    if (err == ESP_OK) s_speech.waiting = true;
    else { eb_speech_stop(&s_speech); s_speech_note = "朗读暂不可用"; publish(); }
}

static void run_action(eb_action_t action)
{
    if (action == EB_ACT_OPEN_BOOK || action == EB_ACT_PAGE_NEXT || action == EB_ACT_PAGE_PREV ||
        action == EB_ACT_PAGE_RESCAN || action == EB_ACT_RELAYOUT || action == EB_ACT_CLOSE_BOOK ||
        action == EB_ACT_XFER_START) speech_stop();
    switch (action) {
    case EB_ACT_SPEECH:
        s_speech_note = NULL;
        if (s_speech.active) { eb_speech_pause(&s_speech); tts_runtime_stop(); }
        else if (s_speech.paused) { s_speech.active = true; s_speech.paused = false; }
        else eb_speech_begin(&s_speech, s_model.page_offset);
        s_reader_full = false;
        break;
    case EB_ACT_SPEECH_STOP:
        speech_stop(); break;
    case EB_ACT_VOLUME_UP:
    case EB_ACT_VOLUME_DOWN:
        tts_runtime_set_volume(eb_speech_volume_step(tts_runtime_volume(), action == EB_ACT_VOLUME_UP ? 1 : -1));
        break;
    case EB_ACT_SPEECH_SPEED:
        if (s_speech.active) { eb_speech_pause(&s_speech); tts_runtime_stop(); }
        s_speech.speed = (s_speech.speed + 1) % 6;
        break;
    ...
    case EB_ACT_XFER_START:
        if (!tts_runtime_shutdown(2000)) { ESP_LOGE(TAG, "audio shutdown timed out"); break; }
...
static void eb_worker(void *arg)
{
    ...
    for (;;) {
        ...
        if (s_speech.active) wait = pdMS_TO_TICKS(50);
        if (xQueueReceive(s_queue, &input, wait) == pdTRUE) {
            ...
            if (s_model.screen == EB_SCREEN_READER && (s_speech.active || s_speech.paused) && key != EB_KEY_OK) {
                tts_runtime_set_volume(eb_speech_volume_step(tts_runtime_volume(), key == EB_KEY_UP ? 1 : -1));
                s_reader_full = false;
                s_chrome_until = s_volume_until = esp_timer_get_time() + EB_CHROME_MS * 1000;
                publish();
                continue;
            }
            ...
            if (s_model.screen == EB_SCREEN_READER && before != EB_SCREEN_READER) {
                s_reader_full = false;
                s_chrome_until = esp_timer_get_time() + EB_CHROME_MS * 1000;
            }
            publish();
            continue;
        }

        if (s_model.screen == EB_SCREEN_READER && !s_reader_full && esp_timer_get_time() >= s_chrome_until) {
            s_reader_full = true; publish();
        }
        if (s_speech.active) { speech_poll(); continue; }
...
static void on_tick(lv_timer_t *timer)
{
    ...
    refresh_footer();
    bool volume_visible = (snapshot.screen == EB_SCREEN_READER && snapshot.volume_until > esp_timer_get_time()) ||
                          (snapshot.screen == EB_SCREEN_MENU && snapshot.volume_edit);
    show(s_volume_box, volume_visible);
    if (volume_visible) {
        lv_label_set_text_fmt(s_volume_label, "音量 %u%%", snapshot.volume);
        lv_bar_set_value(s_volume_bar, (int)snapshot.volume, LV_ANIM_OFF);
    }
}
...
void ebook_prepare(void)
{
    ...
    if (s_storage_ok) eb_fs_seed_sample();
...
void ebook_enter(bool buttons_ok)
{
    ...
    s_volume_box = block(s_screen, 16, 226, 208, 66, UI_INK);
    lv_obj_set_style_border_color(s_volume_box, lv_color_hex(UI_YELLOW), 0);
    lv_obj_set_style_border_width(s_volume_box, 2, 0);
    s_volume_label = ui_pixel_label(s_volume_box, "", &ebook_zh_16, UI_PAPER);
    lv_obj_set_pos(s_volume_label, 10, 8);
    lv_obj_set_width(s_volume_label, 184);
    lv_obj_set_style_text_align(s_volume_label, LV_TEXT_ALIGN_CENTER, 0);
    s_volume_bar = lv_bar_create(s_volume_box);
    lv_obj_set_pos(s_volume_bar, 12, 38);
    lv_obj_set_size(s_volume_bar, 180, 12);
    lv_bar_set_range(s_volume_bar, 0, 100);
    lv_obj_set_style_bg_color(s_volume_bar, lv_color_hex(0x53616A), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(s_volume_bar, LV_OPA_COVER, LV_PART_MAIN);
    lv_obj_set_style_bg_color(s_volume_bar, lv_color_hex(UI_YELLOW), LV_PART_INDICATOR);
    lv_obj_add_flag(s_volume_box, LV_OBJ_FLAG_HIDDEN);
