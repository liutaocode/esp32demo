...
static esp_err_t home(httpd_req_t *r) {
    ...
    int length=snprintf(html,6144,
        ...
        "<label>后端服务器 IP 地址<input name=ip inputmode=decimal placeholder='例如 192.0.2.10' maxlength=15 required></label>"
        ...
        "<small>公网 IP 自动使用加密连接，请勾选访问令牌；免令牌仅用于可信局域网。模型 API Key 仅在后端配置。</small>"
