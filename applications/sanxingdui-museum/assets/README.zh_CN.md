...
- `fonts/sanxingdui_zh_16.c` 是为博物馆云游界面可见中文生成的 16 px、2 bpp LVGL
...
- 使用 `main/sanxingdui_museum.c` 中的可见中文重新生成时，运行
...
- `images/sanxingdui-museum-cover.png` 是为本项目制作的 1086 × 1448 社区封面，
  取材于项目原创的展品构思与运行画面风格，不含博物馆照片、标志或第三方画作。
- `images/sanxingdui-museum-runtime.png` 是 240 × 320 的云游序厅画面抓取，只包含
  应用界面，不含设备或账户标识。
...
- `music/sanxingdui_tts/` 保存 22 段 WAV 源文件，按页面顺序采用
  `01_site_story.wav` 至 `22_location_detail.wav` 的命名方式。
- 全部语音来自项目自编讲解文案，由 `tools/generate_sanxingdui_tts.py` 使用 macOS
  婷婷系统音色生成，不包含博物馆或其他第三方录音。
- WAV 统一为 16 kHz、16 位、单声道 PCM；同一脚本随后将它们压缩打包成
  `main/sanxingdui_tts_audio.c` 的 4-bit IMA-ADPCM 数据。
