#!/usr/bin/env python3
"""Regenerate Cloud Hop's Chinese subset from its UI source."""
...
source = (ROOT / "main/apps/cloud_hop/cloud_hop.c").read_text()
symbols = "".join(sorted({ch for ch in source if ord(ch) > 127}))
output = ROOT / "assets/fonts/cloud_hop_zh_16.c"
...
if args.check:
    ...
    print(f"Cloud Hop font: {len(symbols)} UI characters covered")
    raise SystemExit(0)
subprocess.run([
    ...
    "--lv-include", "lvgl.h", "--lv-font-name", "cloud_hop_zh_16",
    "--lv-fallback", "lv_font_montserrat_14",
    "-o", str(ROOT / "assets/fonts/cloud_hop_zh_16.c"),
], check=True)
