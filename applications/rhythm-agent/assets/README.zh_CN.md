...
- `fonts/rhythm_zh_{12,16,24}.c`：思源黑体中文子集（SIL OFL 1.1），使用 `tools/generate_rhythm_font.py` 生成，覆盖 ASCII 和所有界面字符。
- `music/rhythm_agent/`：四句原创普通话指令，由本机 macOS Tingting 合成，含 WAV、ADPCM 和 SHA-256 清单；无借用音乐或角色录音。用 `tools/generate_rhythm_assets.py --synthesize` 重新生成。
- 三种音符由应用中的 256 项正弦表定点合成，不依赖在线音源。
