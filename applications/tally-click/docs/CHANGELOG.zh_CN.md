...
## 点点有数 1.2.0 — 2026-09-06

- 开机直接计数，删除校时和时间戳，旧计数迁移为最近十条结果。使用预生成 PCM、连续供音和 5 毫秒交叉淡化；播放时暂缓后台计数保存，减少卡顿和切换爆音。

## 点点有数 1.1.0 — 2026-09-06

- 将装饰边框改为深色满屏计数界面，使用高挑矢量数码字，加入数字弹跳、色彩光波、归档扫光和粒子，以及分键／分操作异步音效；检查动画边界、波形包络与音频抢占、故障降级。

## 点点有数 1.0.0 — 2026-09-06

- 新增独立中文计数器，支持按下计数、暂停防误触、归档成功后清零、最近十条带日期持久记录和离线校时；加入故障注入与真实 LVGL 布局检查。设备安装和社区提交待后续进行。
