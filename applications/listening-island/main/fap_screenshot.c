...
static atomic_bool s_capture_frozen;
...
{
    if (atomic_load(&s_capture_frozen)) return;
    ...
    uint32_t source_stride = lv_display_get_render_mode(display) == LV_DISPLAY_RENDER_MODE_PARTIAL
...
static void send_screen(void)
{
    ...
    atomic_store(&s_capture_frozen, true);
    bsp_lvgl_unlock();
    ...
    atomic_store(&s_capture_frozen, false);
