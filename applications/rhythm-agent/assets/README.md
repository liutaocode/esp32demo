...
- `fonts/rhythm_zh_{12,16,24}.c`: Source Han Sans SC subsets (SIL OFL 1.1), generated by `tools/generate_rhythm_font.py`; ASCII and every UI character are included.
- `music/rhythm_agent/`: four original short Mandarin instructions synthesized with the installed macOS Tingting voice. WAV sources, ADPCM pack and SHA-256 manifest are included. No borrowed music or character recordings. Regenerate using `tools/generate_rhythm_assets.py --synthesize`.
- Three note sounds are fixed-point synthesis in the application, using a 256-entry sine table; no streamed media.
