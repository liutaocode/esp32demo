...
- 小猫在呢 0.1.2：喵叫、呼噜与玩具声分组随机且不连续重复；新增双击确认开启的墨镜小猫舞、抬爪动作与原创节拍，三十二秒结束、任意键停舞。加入紧凑 ADPCM 流式解码及手势、声音回归检查。

- 小猫在呢 0.1.1：将过轻的合成声替换为 CC0 真实呼噜与猫叫录音，提高播放电平；加入独立音频任务、有界续播，并在开启声音时播放呼噜供试听。


- 新增「小猫在呢」：中文三键陪伴宠物，支持十一种姿态、名字／毛色／玩具、轻柔合成拟声、持久化小回忆及桌面／随身显示模式。加入实际界面的文字和可见性检查及审核草稿；安装与发布尚待继续。
