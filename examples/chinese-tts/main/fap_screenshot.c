...
__attribute__((weak)) void fap_app_command(const char *line) { (void)line; }

static int discard_log(const char *fmt, va_list args) { (void)fmt; (void)args; return 0; }
...
static void send_screen(void)
{
    ...
    vprintf_like_t previous_log = esp_log_set_vprintf(discard_log);
    s_sent_bytes = 0;
