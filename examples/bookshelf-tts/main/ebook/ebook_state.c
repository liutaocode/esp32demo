...
{
    if (m->volume_edit) {
        if (key == EB_KEY_OK) { m->volume_edit = false; return EB_ACT_REDRAW; }
        return key == EB_KEY_UP ? EB_ACT_VOLUME_UP : EB_ACT_VOLUME_DOWN;
    }
    if (key == EB_KEY_OK && ev == EB_EV_LONG) {
        m->screen = EB_SCREEN_READER;
        return EB_ACT_REDRAW;
    ...
    case EB_MENU_SPEECH:
        m->screen = EB_SCREEN_READER; return EB_ACT_SPEECH;
    case EB_MENU_SPEECH_STOP:
        m->screen = EB_SCREEN_READER; return EB_ACT_SPEECH_STOP;
    case EB_MENU_VOLUME:
        m->volume_edit = true; return EB_ACT_REDRAW;
    case EB_MENU_SPEECH_SPEED:
        return EB_ACT_SPEECH_SPEED;
    ...
    case EB_MENU_BACK:
        m->screen = EB_SCREEN_SHELF; return EB_ACT_CLOSE_BOOK;
    case EB_MENU_THEME:
