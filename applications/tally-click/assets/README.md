...
The three `tally_zh` source files are 2bpp Source Han Sans SC subsets generated by `tools/generate_tally_fonts.py`. This font uses SIL OFL 1.1; its license is in `fonts/tally-licenses/`. Version 1.2 draws original scalable segment numerals and effects directly in LVGL and plays original PCM key cues generated offline by `tools/generate_tally_audio.py`; no bitmap or recorded audio is embedded. The earlier `tally_digits_64.c` Montserrat subset and license remain as unused version-1.0 source material.
