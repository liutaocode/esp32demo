#!/usr/bin/env python3
"""Regenerate Ricochet Rush's Chinese font subsets from its UI source."""
...
source = (ROOT / "main/apps/ricochet_rush/ricochet_rush.c").read_text()
symbols = "".join(sorted({ch for ch in source if ord(ch) > 127}))
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--check", action="store_true")
args = parser.parse_args()
for size in (16,):
    name = f"ricochet_rush_zh_{size}"
    ...
    if args.check:
        ...
        print(f"Ricochet Rush {size}px font: {len(symbols)} UI characters covered")
        continue
    subprocess.run([
