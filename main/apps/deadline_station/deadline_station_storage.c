...
typedef struct { uint8_t data[DS_SAVE_SIZE]; unsigned version; } save_t;
...
static ds_progress_t s_initial = {.seed={2026,9,5,12,0}};
...
ds_progress_t ds_storage_init(void)
{
    ...
    if (nvs_open("deadline_st", NVS_READWRITE, &s_nvs) != ESP_OK) return s_initial;
    uint8_t bytes[DS_SAVE_SIZE];
    size_t length = sizeof(bytes);
    esp_err_t err = nvs_get_blob(s_nvs, "progress_v1", bytes, &length);
    if (err == ESP_OK && length == sizeof(bytes) && ds_decode(&s_initial, bytes)) {
        /* Valid versioned collection. */
    } else if (err != ESP_ERR_NVS_NOT_FOUND) {
        nvs_close(s_nvs); return s_initial;
    }
    s_saves = xQueueCreate(1, sizeof(save_t));
    if (!s_saves || xTaskCreate(worker, "deadline_save", 3072, NULL, 2, NULL) != pdPASS) {
