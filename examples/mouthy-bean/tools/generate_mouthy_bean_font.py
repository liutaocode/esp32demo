#!/usr/bin/env python3
"""Regenerate Mouthy Bean's Chinese font subsets from its UI source."""
...
source = "\n".join(p.read_text() for p in (ROOT / "main").glob("*.c"))
...
for size in (12, 16):
    ...
    if args.check:
        ...
        print(f"Mouthy Bean {size}px font: {len(symbols)} UI characters covered")
        continue
    subprocess.run([
    ...
    output.write_text(output.read_text().replace(str(ROOT), "<repo>").rstrip() + "\n")
