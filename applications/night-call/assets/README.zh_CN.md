...
`fonts/night_call_zh_{12,16,24}.c` 是从实际界面与剧本提取的思源黑体简体中文子集。原字体随 LVGL 分发，遵循其字体许可。`music/night_call/` 包含原创剧本经 macOS Tingting 生成的对白、WAV 源音频、SHA-256 清单与嵌入式 IMA ADPCM 包；未借用角色、音乐或影视录音。生成和校验脚本位于 `tools/generate_night_call_*`。
