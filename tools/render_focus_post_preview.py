...
parser.add_argument('--input', type=Path, default=Path('/tmp/focus-post-previews'))
args = parser.parse_args()
output = ROOT / 'projects/focus-post/assets'
output.mkdir(parents=True, exist_ok=True)
for p in args.input.glob('*.ppm'):
    with Image.open(p) as im:
        im.save(output / (p.stem + '.png'))
names = ['home', 'rule', 'practice-send', 'practice-wait', 'visitor', 'result']
...
ImageDraw.Draw(canvas).text((16, 5), '注意力小邮差 · 本机模拟预览（非设备截图）', font=font, fill='#17202a')
