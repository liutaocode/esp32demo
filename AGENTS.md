...
- Every maintained Markdown document uses English at its default `.md` path and Simplified Chinese in a paired `.zh_CN.md` file. Keep both versions aligned and retain reciprocal language links. The repository-root README is an owner-selected exception: `README.md` is Chinese and `README.en.md` is English; `README.zh_CN.md` retains a compatibility link.
