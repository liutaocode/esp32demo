// main/main.c —— FoloToy AI Passport 海昏侯国遗址博物馆云游应用入口。
#include "bsp_i2c.h"
...
static void on_key(bsp_btn_t btn, bsp_btn_ev_t ev, void *user) {
    ...
    haihunhou_museum_key(btn, ev);
    bsp_lvgl_unlock();
}

void app_main(void) {
    ESP_LOGI(TAG, "FoloToy Haihunhou Museum tour starting");
    ...
    if (bsp_lvgl_lock(1000)) {
        haihunhou_museum_enter(buttons_ok, audio_ok);
        bsp_lvgl_unlock();
    }
