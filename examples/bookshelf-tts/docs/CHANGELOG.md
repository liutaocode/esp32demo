...
- Added two poetry test books, Tang Poems and Song Lyrics, alongside the existing reading sample; existing files and reading progress are preserved.

- Added inline menu volume editing and a transient volume bar during narration.

- Added a separate Pocket Bookshelf TTS edition with continuous Chinese narration,
  sentence-level pause/resume, reading-speed controls, automatic page following
  and silent page turns. During narration, Up/Down adjusts volume and the header
  and footer hide automatically. Existing book storage and reader features are retained.
