...
### Sanxingdui Museum Chinese subset

- `fonts/sanxingdui_zh_16.c` is a 16 px, 2 bpp LVGL subset generated from Noto
...
- Regenerate from the visible Chinese strings in `main/sanxingdui_museum.c` with
  `python3 tools/generate_sanxingdui_font.py /path/to/NotoSansCJKsc-Regular.otf`.
...
### Sanxingdui Museum README artwork

- `images/sanxingdui-museum-cover.png` is the 1086 × 1448 community cover created
...
- `images/sanxingdui-museum-runtime.png` is a 240 × 320 framebuffer capture of the
...
### Sanxingdui Museum offline guide narration

- `music/sanxingdui_tts/` stores 22 source WAV files in page order, named from
  `01_site_story.wav` through `22_location_detail.wav`.
- All clips use project-authored narration synthesized by
  `tools/generate_sanxingdui_tts.py` with the macOS Tingting system voice. No
...
- WAV sources are 16 kHz, 16-bit mono PCM. The same script compresses and packs
  them as 4-bit IMA-ADPCM in `main/sanxingdui_tts_audio.c`.
