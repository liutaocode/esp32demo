...
int main(void) {
    ...
    assert(m.screen == EB_SCREEN_MENU && m.menu_sel == EB_MENU_SPEECH);
    ...
    assert(eb_state_key(&m, EB_KEY_OK, EB_EV_CLICK) == EB_ACT_REDRAW);
    assert(eb_state_key(&m, EB_KEY_OK, EB_EV_CLICK) == EB_ACT_SPEECH);
    assert(m.screen == EB_SCREEN_READER);
    assert(eb_state_key(&m, EB_KEY_OK, EB_EV_CLICK) == EB_ACT_REDRAW);
    m.menu_sel=EB_MENU_SPEECH_STOP;
    assert(eb_state_key(&m, EB_KEY_OK, EB_EV_CLICK) == EB_ACT_SPEECH_STOP);
    assert(m.screen == EB_SCREEN_READER);

    m.screen = EB_SCREEN_MENU; m.menu_sel = EB_MENU_VOLUME;
    assert(eb_state_key(&m, EB_KEY_OK, EB_EV_CLICK) == EB_ACT_REDRAW && m.volume_edit);
    assert(eb_state_key(&m, EB_KEY_UP, EB_EV_CLICK) == EB_ACT_VOLUME_UP);
    assert(eb_state_key(&m, EB_KEY_DOWN, EB_EV_LONG) == EB_ACT_VOLUME_DOWN);
    assert(m.menu_sel == EB_MENU_VOLUME && m.screen == EB_SCREEN_MENU);
    assert(eb_state_key(&m, EB_KEY_OK, EB_EV_LONG) == EB_ACT_REDRAW && !m.volume_edit);
    assert(m.screen == EB_SCREEN_MENU);
    assert(eb_state_key(&m, EB_KEY_OK, EB_EV_LONG) == EB_ACT_REDRAW && m.screen == EB_SCREEN_READER);
