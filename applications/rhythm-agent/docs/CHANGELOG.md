...
## 2026-09-05 — Rhythm Agent

- Let correct key sequences advance training without a timing gate or deadline; score adjacent intervals independently and explain retry causes. Add slow-training and tempo-drift regression coverage.
- Match the on-screen pads and instructions to the physical UP / DOWN / OK order.
- Add an offline Chinese rhythm game with training and challenge modes, seeded replay, Mandarin cues, input-timestamp scoring and persistent challenge records.
- Add host LVGL layout checks and audio-stream tests; keep serial capture ready for later device acceptance.
